...
!  real(ESMF_KIND_R8), parameter :: sbf    = 300.D0  ! seconds between frames [s]
  ...
  type InternalStateStruct
    ...
    type(ESMF_RouteHandle)        :: haloTransHandle
  ...
  recursive subroutine SetServices(gcomp, rc)
    ...
    call ESMF_MethodAdd(gcomp, label=model_label_CheckImport, &
      index=1, userRoutine=CheckImport, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  recursive subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
    ...
    call ESMF_AttributeGet(gcomp, name="NestingGeneration", &
  ...
  recursive subroutine InitializeP1(gcomp, importState, exportState, clock, rc)
    ...
    call ESMF_AttributeGet(gcomp, name="Nestling", &
    ...
    if (nestingGeneration == 0) then
      ...
      ! This is a nest domain
    
      if ((nestingGeneration == 1) .and. (nestling == 0)) then
      ...
      else if ((nestingGeneration == 1) .and. (nestling == 1)) then
        ! This is nestling 1 in the first nest generation
      
        dx = dxTop / 3.d0 ! three times the resolution in x
        ! -> create the Grid
        is%wrap%grid = ESMF_GridCreateNoPeriDim( &
          ! Define a regular distribution (no regDecomp arg -> divide along i w/ DEs)
          minIndex=(/1,1/), &           ! begin of index space
        ...
        call ESMF_GridGetCoord(is%wrap%grid, coordDim=1, farrayPtr=coordPtr, rc=rc)
        do i=lbound(coordPtr,1),ubound(coordPtr,1)
          coordPtr(i) = 66 * dxTop + i * dx  ! shifted by 2/3 of parent domain
        enddo
        ! y-coordinates
        call ESMF_GridGetCoord(is%wrap%grid, coordDim=2, farrayPtr=coordPtr, rc=rc)
        do i=lbound(coordPtr,1),ubound(coordPtr,1)
      ...
      else if ((nestingGeneration == 2) .and. (nestling == 0)) then
        ! This is nestling 0 in the second nest generation
      
        dx = dxTop / 4.d0 ! four times the resolution in x
        ! -> create the Grid
        is%wrap%grid = ESMF_GridCreateNoPeriDim( &
          ! Define a regular distribution (no regDecomp arg -> divide along i w/ DEs)
          minIndex=(/1,1/), &           ! begin of index space
        ...
        call ESMF_GridGetCoord(is%wrap%grid, coordDim=1, farrayPtr=coordPtr, rc=rc)
        do i=lbound(coordPtr,1),ubound(coordPtr,1)
          coordPtr(i) = 70 * dxTop + i * dx  ! shifted by 70% of parent domain
        enddo
        ! y-coordinates
        call ESMF_GridGetCoord(is%wrap%grid, coordDim=2, farrayPtr=coordPtr, rc=rc)
        do i=lbound(coordPtr,1),ubound(coordPtr,1)
      ...
      is%wrap%fieldTrans = ESMF_FieldCreate(grid=is%wrap%gridTrans, &
        ...
        indexflag=ESMF_INDEX_GLOBAL, name="dens", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return
      
      ! -> precompute halo operation for transfer Field
      call ESMF_FieldHaloStore(is%wrap%fieldTrans, &
        routehandle=is%wrap%haloTransHandle, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return
...
!      return
  ...
  subroutine SetClock(gcomp, rc)
    ...
    else if (nestingGeneration == 1) then
      dt = dtTop / childSteps ! child domain time step generation 1
    else if (nestingGeneration == 2) then
      dt = dtTop / childSteps**2 ! child domain time step generation 2
    endif
...
!      ringTime = currTime + callerTimeStep - stabilityTimeStep, &
    ...
    if (nestingGeneration == 0) then
      ...
      ! This is the child domain nesting generation 1
      ...
      ! This is the child domain nesting generation 2
      call ESMF_TimeIntervalSet(alarmInterval, s_r8 = dtTop/childSteps, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return
  ...
  recursive subroutine ModelAdvance(gcomp, rc)
    ...
    if (ESMF_AlarmIsRinging(is%wrap%timeToTransfer)) then
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        ...
        file=__FILE__)) &
        return
      call ESMF_FieldHalo(is%wrap%fieldTrans, &
        routehandle=is%wrap%haloTransHandle, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
...
!        return
  ...
  recursive subroutine CheckImport(gcomp, rc)
    ...
    ! See if this is a child nest domain first time coming in after coupling
    if (ESMF_AlarmIsRinging(is%wrap%timeToTransfer)) then
      ! -> this means that the import Field timestamp must be checked for match
      ! -> with current time on model clock
    
      call ESMF_LogWrite("checking time stamp on import", ESMF_LOGMSG_INFO, &
        rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
