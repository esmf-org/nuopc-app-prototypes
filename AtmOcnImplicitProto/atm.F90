module ATM
  ...
  !   Run() phase 1: down sweep
  !   Run() phase 2: up sweep
  !-----------------------------------------------------------------------------
  ...
  subroutine SetServices(gcomp, rc)
    ...
    call ESMF_GridCompSetEntryPoint(gcomp, ESMF_METHOD_RUN, &
      userRoutine=Run_sweep_dn, phase=2, rc=rc)   ! DOWN SWEEP on phase 2
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_GridCompSetEntryPoint(gcomp, ESMF_METHOD_RUN, &
      userRoutine=Run_sweep_up, phase=3, rc=rc)   ! UP SWEEP on phase 3
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine Run_sweep_dn(gcomp, importState, exportState, clock, rc)
    type(ESMF_GridComp)  :: gcomp
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
    integer, intent(out) :: rc
    ...
    print *, "ATM Down Sweep is executing"
    
    call NUOPC_ClockPrintCurrTime(clock, "Clock inside of ATM Down Sweep:  ", &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
  ...
  subroutine Run_sweep_up(gcomp, importState, exportState, clock, rc)
    type(ESMF_GridComp)  :: gcomp
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
    integer, intent(out) :: rc
    ...
    print *, "ATM Up Sweep is executing"
    
    call NUOPC_ClockPrintCurrTime(clock, "Clock inside of ATM Up Sweep:  ", &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
