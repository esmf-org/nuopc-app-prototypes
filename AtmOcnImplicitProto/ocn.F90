module OCN
  ...
  !   Run() phase 1: slow processes
  !   Run() phase 2: fast processes
  !-----------------------------------------------------------------------------
  ...
  subroutine SetServices(gcomp, rc)
    ...
    call ESMF_GridCompSetEntryPoint(gcomp, ESMF_METHOD_RUN, &
      userRoutine=Run_fast_procs, phase=2, rc=rc)   ! FAST processes on phase 2
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_GridCompSetEntryPoint(gcomp, ESMF_METHOD_RUN, &
      userRoutine=Run_slow_procs, phase=3, rc=rc)   ! SLOW processes on phase 3
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine Run_fast_procs(gcomp, importState, exportState, clock, rc)
    type(ESMF_GridComp)  :: gcomp
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
    integer, intent(out) :: rc
    ...
    print *, "OCN Fast Processes executing"
    
    call NUOPC_ClockPrintCurrTime(clock, "Clock inside of OCN Fast Procs:  ", &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
  ...
  subroutine Run_slow_procs(gcomp, importState, exportState, clock, rc)
    type(ESMF_GridComp)  :: gcomp
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
    integer, intent(out) :: rc
    ...
    print *, "OCN Slow Processes executing"
    
    call NUOPC_ClockPrintCurrTime(clock, "Clock inside of OCN Slow Procs:  ", &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
