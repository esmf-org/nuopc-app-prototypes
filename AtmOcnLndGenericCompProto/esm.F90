module ESM
  ...
  subroutine SetModelServices(gcomp, rc)
    ...
    ! SetServices for LND
    call ESMF_GridCompSetServices(is%wrap%lnd, lndSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
#define WITHCONNECTORS
    ...
    call ESMF_CplCompSetServices(is%wrap%atm2lnd, cplSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    ! SetServices for ocn2atm
    call ESMF_CplCompSetServices(is%wrap%lnd2atm, cplSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
