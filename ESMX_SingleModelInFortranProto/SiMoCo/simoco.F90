...
module SIMOCO
  ...
  !-----------------------------------------------------------------------------
  ! SiMoCo (Simple Model Component) in Fortran
  !-----------------------------------------------------------------------------
...
#define WITHEXPORTFIELDS
  ...
  subroutine Advance(model, rc)
    ...
    call ESMF_ClockPrint(clock, options="currTime", &
      preString="------>Advancing SIMOCO from: ", unit=msgString, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
subroutine SetVM(comp, rc)
  use ESMF
  use SIMOCO, only: SetVMModule => SetVM
...
subroutine SetServices(comp, rc)
  use ESMF
  use SIMOCO, only: SetServicesModule => SetServices
  type(ESMF_GridComp) :: comp
  integer, intent(out) :: rc
