...
#define COMPLIANCE_CHECK_DRIVER__off
  ...
  subroutine SetModelServices(driver, rc)
    ...
    if (trim(enable_field_mirroring)=="yes") then
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        ...
        file=__FILE__)) &
        return  ! bail out
      call NUOPC_CompAttributeSet(drv2comp, name="Verbosity", value="low", &
        rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        ...
        file=__FILE__)) &
        return  ! bail out
      call NUOPC_CompAttributeSet(comp2drv, name="Verbosity", value="low", &
        rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
    ...
    if (trim(enable_compliance_check)=="yes") then
      ...
      if (trim(enable_field_mirroring)=="yes") then
#if 0
        call ESMF_CplCompSetServices(drv2comp, &
          userRoutine=registerIC_Connector, userRc=urc, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
        if (ESMF_LogFoundError(rcToCheck=urc, msg=ESMF_LOGERR_PASSTHRU, &
          ...
          file=__FILE__)) &
          return  ! bail out
        call ESMF_CplCompSetServices(comp2drv, &
          userRoutine=registerIC_Connector, userRc=urc, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
  ...
  subroutine InitializeP1(driver, importState, exportState, clock, rc)
    ...
    if (trim(enable_field_mirroring)=="yes") then
      call NUOPC_DriverGetComp(driver, compLabel=compName, comp=comp, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
    ...
    do i=lbound(itemNameList,1), ubound(itemNameList,1)
      if (itemTypeList(i)==ESMF_STATEITEM_FIELD) then
        ...
        if (trim(transferGeom)=="accept") then
          ...
          if (associated(ungriddedLBound) .and. associated(ungriddedUBound)) &
            then
...
#define DEBUG_DISTGRID
          ...
          write(valueString, *) "DistGrid minIndexPTile(:,1) = ", &
            minIndexPTile(:,1)
          ...
          write(valueString, *) "DistGrid maxIndexPTile(:,1) = ", &
            maxIndexPTile(:,1)
          call ESMF_LogWrite(valueString, ESMF_LOGMSG_INFO, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
          ...
          call ESMF_LogWrite("CANNOT complete mirrored field: "//&
            itemNameList(i), ESMF_LOGMSG_INFO, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
  ...
  subroutine TimestampMirroredFields(driver, importState, exportState, clock, &
    rc)
    type(ESMF_GridComp)  :: driver
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
  ...
  subroutine SetRunSequence(driver, rc)
    ...
    call NUOPC_DriverAddRunElement(driver, slot=1, srcCompLabel=compName, &
      dstCompLabel="explorerDriver", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) &
      return  ! bail out

    call NUOPC_DriverAddRunElement(driver, slot=1, compLabel=compName, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) &
      return  ! bail out
  ...
  recursive subroutine ModifyCplList(driver, importState, exportState, clock, &
    rc)
    type(ESMF_GridComp)  :: driver
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
