...
module TAWAS
  ...
  !-----------------------------------------------------------------------------
  ! TAWAS: The Absolute Worst Atmosphere Stand-in
  !-----------------------------------------------------------------------------
...
#define WITHIMPORTFIELDS_LOCSTREAM_off
  ...
  subroutine Realize(model, rc)
    ...
    do i=clb(1),cub(1)
       ...
       lat(i)=0.0
       fptr(i)=0.0 ! Init to 0.0
       mask(i)=0
       ! Mask out range
       ! (Same range as in ocn.F90)
...
!$  use omp_lib
    ...
    type(ESMF_Field)            :: field
    real(ESMF_KIND_R8), pointer :: fptrR81D(:)
    real(ESMF_KIND_R8), pointer :: fptrR82D(:,:)
...
!$omp end parallel
    ...
    call ESMF_ClockPrint(clock, options="currTime", &
      preString="------>Advancing TAWAS from: ", unit=msgString, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    ! do something with sst
    call ESMF_StateGet(importState, itemName="sst", field=field, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_FieldGet(field, farrayPtr=fptrR82D, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    fptrR82D = fptrR82D * fptrR82D
    print *, maxval(fptrR82D)

    ! do something with sss
    call ESMF_StateGet(importState, itemName="sss", field=field, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_FieldGet(field, farrayPtr=fptrR81D, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    fptrR81D = fptrR81D * fptrR81D
    print *, maxval(fptrR81D)
...
subroutine SetVM(comp, rc)
  use ESMF
  use TAWAS, only: SetVMModule => SetVM
...
subroutine SetServices(comp, rc)
  use ESMF
  use TAWAS, only: SetServicesModule => SetServices
  type(ESMF_GridComp) :: comp
  integer, intent(out) :: rc
