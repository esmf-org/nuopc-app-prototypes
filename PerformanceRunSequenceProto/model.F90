  ...
  subroutine SetServices(model, rc)
   ...
   call NUOPC_CompSetEntryPoint(model, ESMF_METHOD_INITIALIZE, &
    ...
    do i=1, 100
      ...
      write (runLabel, "(A,I3.3)") "myRunLabel", i
      
      call NUOPC_CompSetEntryPoint(model, ESMF_METHOD_RUN, &
!        phaseLabelList=(/trim(runLabel)/), userRoutine=myRunRoutine, rc=rc)
        phaseLabelList=(/trim(runLabel)/), userRoutine=model_routine_Run, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
...
#define WITHIMPORTFIELDS___disable
  ...
  subroutine myRunRoutine(model, importState, exportState, clock, rc)
    type(ESMF_GridComp)  :: model
    type(ESMF_State)     :: importState, exportState
    type(ESMF_Clock)     :: clock
    integer, intent(out) :: rc
