module driverComp
  ...
  subroutine SetModelCount(gcomp, rc)
    ...
    ! set the modelCount for a single model component with a single nestling
  ...
  subroutine SetModelServices(gcomp, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=localrc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      rcToReturn=rc)) &
      return  ! bail out
    call ESMF_GridCompSet(is%wrap%modelComp(2), name="advectDiff_Nestling0", &
      rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
...
!!!!!      
    ...
    call ESMF_AttributeSet(is%wrap%modelComp(1), name="NestingGeneration", &
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      file=__FILE__)) &
      return  ! bail out
    call ESMF_AttributeSet(is%wrap%modelComp(1), name="Nestling", &
      convention="NUOPC", purpose="General", &
