...
!      return  ! bail out
  ...
  subroutine InitializeRealize(model, importState, exportState, clock, rc)
    ...
    gridIn = ESMF_GridCreate1PeriDimUfrm(maxIndex=(/100, 150/), &
      ...
      staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER/), name="OCN-GridIn", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
!      return  ! bail out
    ...
    gridOut = ESMF_GridCreate1PeriDimUfrm(maxIndex=(/100, 120/), &
      ...
      staggerLocList=(/ESMF_STAGGERLOC_CENTER, ESMF_STAGGERLOC_CORNER/), name="OCN-GridOut", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
