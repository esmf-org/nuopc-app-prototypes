module ATM
  ...
  subroutine InitializeP4(model, importState, exportState, clock, rc)
    ...
    !NOTE: the LocStream object that is referenced by the "pmsl" Field.
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      file=__FILE__)) &
      return  ! bail out
    write (msgString,*) "ATM - InitializeP4: transferred LocStream name = ", name
    ...
    ! access localDeCount of the final LocStream
    call ESMF_LocStreamGet(locStream, localDeCount=localDeCount, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
  ...
  subroutine InitializeP5(model, importState, exportState, clock, rc)
    ...
    ! the transferred LocStream is already set, allocate memory for data by complete
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      ...
      file=__FILE__)) &
      return  ! bail out
    write (msgString,*) "ATM - InitializeP5: transferred LocStream name = ", &
      name
    call ESMF_LogWrite(msgString, ESMF_LOGMSG_INFO, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
