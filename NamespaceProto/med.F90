module MED
  ...
  integer, parameter    :: atmCount=2 ! number of ATMs this MED expects to interact with
...
#define WITHIMPORTFIELDS
    ...
    do i=1, atmCount
      ...
      state = ESMF_StateCreate(name="Import ATM #"//trim(adjustl(iString)), &
        rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
  ...
  subroutine InitializeP2(gcomp, importState, exportState, clock, rc)
    ...
    do i=1, atmCount
      ...
      call ESMF_StateGet(importState, &
        itemName="Import ATM #"//trim(adjustl(iString)), nestedState=state, &
        rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
